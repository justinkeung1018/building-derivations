...
it("warns users about left-recursive rules", () => {
  cy.getBySel("syntax-def-0").click().focus().type("abcdef").should("have.value", "abcdef").blur();
  cy.getBySel("add-syntax-button").click();
  cy.getBySel("syntax-placeholders-1").click().focus().type("A").should("have.value", "A").blur();
  cy.getBySel("syntax-def-1").click().focus().type("Abcd").should("have.value", "Abcd").blur();
...
it("lets me add multiple rules", () => {
  for (let i = 0; i < 10; i++) {
    ...
    cy.getBySel(`syntax-def-${i}`).click().focus().type(String(i)).should("have.value", String(i)).blur();
  }
  cy.getBySel("apply-syntax-button").click();
