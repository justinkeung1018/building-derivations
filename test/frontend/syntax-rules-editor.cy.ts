...
it("warns users about left-recursive rules", () => {
  cy.getBySel("syntax-def-0").type("abcdef");
  cy.getBySel("add-syntax-button").click();
  cy.getBySel("syntax-placeholders-1").type("A");
  cy.getBySel("syntax-def-1").type("Abcd");
...
it("lets me add multiple rules", () => {
  for (let i = 0; i < 10; i++) {
    if (i > 0) {
      cy.getBySel("add-syntax-button").click();
      cy.getBySel(`syntax-placeholders-${i}`).type("a" + String(i));
    }
    cy.getBySel(`syntax-def-${i}`).type(String(i));
  }
  cy.getBySel("apply-syntax-button").click();
