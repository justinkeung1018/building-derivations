describe("LaTeX alternatives are recognised identically", () => {
  it("recognises different names for inference rules", () => {
    ...
    cy.getBySel("tree-conclusion-0").type("x, y, z, y |- (x -> y)");
    cy.getBySel("tree-rule-0").type("\\to I"); // Change this line to e.g. \rightarrow I
    cy.getBySel("tree-add-premise-button-0").click();
    cy.getBySel("tree-conclusion-1").type("x, x, y, z, y |- y");
