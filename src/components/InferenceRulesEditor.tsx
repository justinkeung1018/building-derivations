...
function PremisesEditor({ editing, rule, index, setInferenceRules }: DefinitionEditorProps) {
  if (!editing) {
    return <MathJax>{`\\(${rule.premises.map((premise) => premise.definitionSanitised).join(" \\quad ")}\\)`}</MathJax>;
  }
...
function DefinitionEditor(props: DefinitionEditorProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-center">
        {editing ? <Input /> : <MathJax>{`\\(${rule.conclusion.definitionSanitised.join(" ")}\\)`}</MathJax>}
      </div>
    </div>
  );
}
...
function InferenceRulesEditor(props: InferenceRulesEditorProps) {
  const { inferenceRules, setInferenceRules } = props;
